...
file_en = "gsoc/siddhant/DataAug/Data/data.en"
file_sp = "gsoc/siddhant/DataAug/Data/data.sparql"
...
shuffled_en, shuffled_sp = text_en[549952:549999], text_sp[549952:549999]
...
f = open("gsoc/siddhant/DataAug/Syntax-aware/dropped_data.en", "w+")
for i in range(len(en_sp_dropout)):
    f.write(en_sp_dropout[i][0]+"\n")

f.close()

f = open("gsoc/siddhant/DataAug/Syntax-aware/dropped_data.sparql", "w+")
for i in range(len(en_sp_dropout)):
...
f = open("gsoc/siddhant/DataAug/Syntax-aware/replaced_data.en", "w+")
for i in range(len(en_sp_replacement)):
    f.write(en_sp_replacement[i][0]+"\n")

f.close()

f = open("gsoc/siddhant/DataAug/Syntax-aware/replaced_data.sparql", "w+")
for i in range(len(en_sp_replacement)):
